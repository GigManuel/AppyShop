...
$(document).ready(function() {
	...
	// demarrage quiz
		$('.panel.question1 .reponses ul li').css('margin-top','-300px');
	...
	$('.bouton-validation').click(function(){
		...
		$('.question'+currentQuestion+' .reponses ul li a').each(function(){
			if (($(this).attr('class') == 'v') || ($(this).attr('class') == 'v choix')) {
				$(this).css({'background-color':'#99d740','color':'#fff'});
			}else{
				$(this).css({'background-color':'#cc2222','color':'#fff'});	
			}
		});
		...
		$('li a.choix').each(function(index) {
			...
			if($(this).hasClass('f')) {
				...
				// message échec
				$('.message-reponse').css('background-color','#cc2222');
				...
				score = score + 1;
				// message réussite
				$('.message-reponse').css('background-color','#99d740');
				$('.message-reponse').fadeIn('fast', function(){
		...
		if(nbreCheck == countQuestion+1) {
			...
			$('.final').delay(3000).fadeIn().text('Tu a terminé le questionnaire ! Ton score est de '+score+'/'+(countQuestion+1)+' soit '+percent.toFixed(0)+'% de bonnes réponses');
			
			
			if(percent > 50){
				$('.score').css('color','#99d740');
			}else{
				$('.score').css('color','#cc2222');
			}
		}
